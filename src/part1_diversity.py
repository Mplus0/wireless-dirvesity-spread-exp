...
def selection_combining(received, channel):
    ...
    branch_power = np.abs(channel) ** 2
    best_branch = np.argmax(branch_power, axis=0)
    symbol_index = np.arange(received.shape[1])
    return received[best_branch, symbol_index] / channel[best_branch, symbol_index]
...
def simulate_diversity_ber(snr_db_values, num_bits=4000, num_branches=2, seed=2026):
    ...
    for index, snr_db in enumerate(snr_db_values):
        ...
        ber_curves['单分支'].append(calculate_ber(bits, bpsk_demodulate(single_branch)))
...
def equal_gain_combining(received, channel):
    ...
    corrected = phase_weights * received
    denominator = np.sum(np.abs(channel), axis=0)
    return np.sum(corrected, axis=0) / np.maximum(denominator, 1e-12)
